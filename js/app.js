...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    //url of imgages of products
    const image = `https://fakestoreapi.com/img/.jpg`;
    //create div
...
<button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
      <button onclick ="detail(${product.id})" id="details-btn" class="btn btn-danger">Details</button>
</div>`;
...
const updatePrice = (id, value) => {
  ...
  //here we get 2 decimel 
  document.getElementById(id).innerText = total.toFixed(2);
  updateTotal();
};
//show deatils of products
const detail = (id) => {
  const detailOfDiv = `https://fakestoreapi.com/products/${id}`
  console.log(detailOfDiv)
  fetch(detailOfDiv)
    .then(res => res.json())
